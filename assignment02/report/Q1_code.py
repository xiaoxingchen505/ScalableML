...
rawdata = spark.read.csv('HIGGS.csv.gz')
...
import time
print('')
print('----Start to find best parameters using accuracy metric----')
...
(trainingData, testData) = small_data.randomSplit([0.7, 0.3],47)
trainingData.cache()
testData.cache()
...
print('-------Random Forest best model parameter for accuracy metric-----------')
...
print('---------------GBT best model parameter for accuracy metric-------------')
...
print('----Start to find best parameters using auc metric----')
evaluator = BinaryClassificationEvaluator(labelCol="labels",rawPredictionCol='rawPrediction', metricName="areaUnderROC")
crossval_rf = CrossValidator(estimator=pipeline_rf,
                          estimatorParamMaps=paramGrid_rf,
                          evaluator=evaluator,
...
start = time.time()
auc_rfModel = crossval_rf.fit(trainingData)
rf_predictions = auc_rfModel.transform(testData)
end = time.time()
print('Random Forest Finding best model execution time:',end-start)

start = time.time()
auc_gbtModel = crossval_gbt.fit(trainingData)
gbt_predictions = auc_gbtModel.transform(testData)
end = time.time()
print('GBT Finding best model execution time:',end-start)
...
bestPipeline_rf, bestPipeline_gbt = auc_rfModel.bestModel, auc_gbtModel.bestModel

bestModel_rf, bestModel_gbt = bestPipeline_rf.stages[0], bestPipeline_gbt.stages[0]

#output best parameters for each model.
print('-------Random Forest best model parameter for auc metric-----------')
print('numTrees - ', bestModel_rf.getNumTrees)
print('maxDepth - ', bestModel_rf.getOrDefault('maxDepth'))
print('maxBins - ', bestModel_rf.getOrDefault('maxBins'))
print('------------------------------------------')
...
print('---------------GBT best model parameter for auc metric-------------')
print('maxIter - ', bestModel_gbt.getOrDefault('maxIter'))
print('maxDepth - ', bestModel_gbt.getOrDefault('maxDepth'))
print('maxBins - ', bestModel_gbt.getOrDefault('maxBins'))
print('------------------------------------------')
...
print('----------------------------------')
print('Start to train dataset with best parameter...')
print('----------------------------------')
...
(trainingData, testData) = data.randomSplit([0.7, 0.3],47)
